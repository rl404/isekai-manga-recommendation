...
function setArea(qID) {
  ...
  q.options.forEach((o) => {
    oArea += `<div class='row option-row'>`;
    if (o.manga) {
      oArea += `<button class='btn btn-outline-light btn-block option-button' onclick='setManga(${
        o.manga
      })' data-toggle="modal" data-target="#manga-modal">${Mangas[o.manga].title}</button>`;
    } else {
